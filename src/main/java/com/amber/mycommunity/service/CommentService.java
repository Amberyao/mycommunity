...
import com.amber.mycommunity.enums.NotificationStatusEnum;
...
import com.amber.mycommunity.mapper.*;
import com.amber.mycommunity.model.*;
...
public class CommentService {
    ...
    public void insert(Comment comment, User commentator) {
        ...
        if (comment.getType() == CommentTypeEnum.COMMENT.getType()) {
            ...
            // 创建通知
            //comment是新添加的回复 comment是dbcomment的回复  因此comment回复后需要通知dbcomment的拥有者也就是recevier 而comment的拥有者就是通知者notifier
            ...
            //存在插入成功 但没有写进去数量的情况  用事务做处理(保证同时成功 成功失败)
            commentMapper.insert(comment);
